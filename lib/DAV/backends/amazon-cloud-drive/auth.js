...
util.inherits( AmazonAuth , events.EventEmitter );

// Constants
AmazonAuth.ENDPOINT_UPDATE_CACHE_TIME = 1000 * 60 * 60 * 24 * 5; // 5 days
...
AmazonAuth.prototype.getToken = function getToken( callback ) {
    ...
    var tokenExpiresIn = this.token.expires_in - 30;
    var tokenExpiresInMs = tokenExpiresIn * 1000;
    var tokenExpiresOnMs = this.tokenLastUpdated + tokenExpiresInMs;
    if( tokenExpiresOnMs < Date.now() ) {
...
AmazonAuth.prototype.generateAuthUrl = function generateAuthUrl() {
    ...
    var authUrlObject = {
        ...
        search: querystring.stringify( {
            ...
            scope: this.scope,
            response_type: this.client_secret ? 'code' : 'token',
...
// Helper method
AmazonAuth.prototype.getHeadersAndEndpoint = function getHeadersAndEndpoint( callback ) {
    this.getHeadersForRequest( function( error , headers ) {
        if( error ) {
            return callback( error );
        }

        this.getEndpoint( function( error , endpoint ) {
            if( error ) {
                return callback( error );
            }

            callback( null , headers , endpoint );
        } );
    }.bind( this ) );
};

...
var jsDAV_ACD_File = module.exports = jsDAV_ACD_Node.extend( jsDAV_File , {
            ...
            formData: {
                content: {
                    ...
                    options: {
                        ...
                        contentType: mime.lookup( this.metadata.name ),
                    },
                },
            },
        };
    ...
    get: function( cbacdgetfile ) {
        ...
        jsDAV_ACD_Util.acdRequest( this.tree.amazonAuth , 'contentUrl' , { url: '/nodes/' + this.metadata.id + '/content' } , cbacdgetfile , function( error , response , body ) {
            if( error ) {
                return cbacdgetfile( new Exc.jsDAV_Exception( 'Error while getting ' + nicePath ) );
            }

            if( 200 !== response.statusCode ) {
                var exc = new Exc.jsDAV_Exception( 'Error getting ' + nicePath );
                exc.code = res.statusCode;
                return cbacdgetfile( exc );
            }

            return cbacdgetfile( null , body );
        } );
    },
} );

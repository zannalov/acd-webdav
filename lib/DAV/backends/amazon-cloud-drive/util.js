...
module.exports.acdRequest = function acdRequest( opts ) {
    ...
    function _acdRequest() {
        opts.amazonAuth.getHeadersAndEndpoint( function( error , headers , endpoint ) {
            if( error ) {
                return opts.directCallback( new Exc.jsDAV_Exception( 'Error encountered while trying to get headers or endpoint prior to performing request' ) );
            }
            ...
            function shouldRetry( err , res ) {
                return Boolean( !err && -1 !== [ 401 , 429 , 500 , 503 ].indexOf( res.statusCode ) && opts.maxAttempts > attempt );
            }

...
var jsDAV_ACD_Tree = module.exports = jsDAV_Tree.extend( {
    ...
    move: function( sourcePath , destinationPath , cbacdmove ) {
        ...
        this.getNodeForPath( sourcePath , afterSourceNode.bind( this ) );

        function afterSourceNode( err , node ) {
            if( err ) {
            ...
            sourceNode = node;
            this.getNodeForPath( sourceDirPath , afterSourceDirNode.bind( this ) );
            this.getNodeForPath( destinationDirPath , afterDestinationDirNode.bind( this ) );
        }

        function afterSourceDirNode( err , node ) {
            if( err ) {
                return cbacdmove( err );
            }
            ...
            afterAllLoaded.call( this );
        }

        function afterDestinationDirNode( err , node ) {
            if( err ) {
                return cbacdmove( err );
            }

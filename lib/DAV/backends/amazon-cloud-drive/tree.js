...
var jsDAV_ACD_Tree = jsDAV_Tree.extend( {
    ...
    invalidateParentDirCachedChildren: function( realPath ) {
        var dirPath = Util.splitPath( realPath )[ 0 ];
        if( dirPath !== realPath && this.cachedNodesByRealPath[ dirPath ] ) {
            this.cachedNodesByRealPath[ dirPath ].cachedChildren = null;
        }
    },
    ...
    _fetchNextPageOfChanges: function() {
        ...
        var handleResponse = function( err , res , body ) {
            ...
            body.nodes.forEach( function( nodeUpdate ) {
                if( 'TRASH' === nodeUpdate.status ) {
                    if( this.cachedNodesById[ nodeUpdate.id ] ) {
                        var trashedNodeRealPath = this.cachedNodesById[ nodeUpdate.id ].realPath;
                        this.invalidateParentDirCachedChildren( trashedNodeRealPath );
                        delete this.cachedNodesByRealPath[ trashedNodeRealPath ];
                        delete this.cachedNodesById[ nodeUpdate.id ];
                    }
                } else {
                    ( nodeUpdate.parents || [] ).forEach( function( parentId ) {
                        if( this.cachedNodesById[ parentId ] ) {
                            this._initNode( this.cachedNodesById[ parentId ].realPath.replace( /\/+$/ , '' ) + '/' + nodeUpdate.name , nodeUpdate );
                        }
                    }.bind( this ) );
                }
            }.bind( this ) );
    ...
    _initNode: function( realPath , metadata ) {
        ...
        if( node ) {
            ...
            this.invalidateParentDirCachedChildren( realPath );
        }
